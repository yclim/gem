...
const GroupCard: FunctionComponent<IProps> = ({ group, updateGroups, focusGrp, setFocusGrp }) => {
    ...
    function renderGroupRule(ri: Rule) {
        return (
            <Popover key={`popover-${ri.label}`} position={Position.RIGHT_TOP} content={renderMenu()}>
                <Button
                    key={`button-${ri.label}`}
    ...
    return (
        <Card
            ...
            className={`card group-card ${focusGrp === group ? "group-card-focus": ""}`}
            onClick={handleClick}
        >
            <div className="rule-topbar"> <Icon icon="cross"/> </div>
            <div className="rule-header">
                <div className="label"><EditableText value={grp.groupName}  onChange={e => handleChangeGroupName(e)} onConfirm={() => handleConfirm()}/></div>
