 ...
 *
 * CAR_ID	CAR_BRAND	CAR_MODEL	YEAR	CAPACITY
 * 0	    Rover	    Trade	    1980	2.7
 *
 * Second sheet contains car dealer data that looks like:
 *
 * CAR_ID	CAR_DEALER	    PRICE
 * 0	    Best Auto	    108000
 **/
public class ExcelFileGenerator {

    final static String[] carBrands = ("Seat Renault Peugeot Dacia Citroën Opel Alfa Romeo Škoda Chevrolet Porsche Honda" +
    ...
    final static String[] carModels = ("Alhambra Altea Arosa Cordoba Exeo Ibiza Leon Inca Toledo Captur " +
            ...
            "Cabrio City-Coupé Forfour Roadster").split("\\s+");
    ...
    public static void generateCarsExcelFiles(int numOfFiles, Path dest) throws IOException {
        ...
        for (int i = 0; i < numOfFiles; i++) {
            ...
            FileOutputStream outputStream = new FileOutputStream(Paths.get(dest.toString(), filename).toFile());
    ...
    public static List<List<String>> generateSheetOneDataTables(int rows) {
        ...
        List<String> headers =
                Arrays.asList("CAR_ID", "CAR_BRAND", "CAR_MODEL", "YEAR", "CAPACITY");
        table.add(headers);
    ...
    public static List<List<String>> generateSheetTwoDataTables(int rows) {
        List<List<String>> table = new ArrayList<>();
        List<String> headers =
                Arrays.asList("CAR_ID", "CAR_DEALER", "PRICE");
        table.add(headers);
        for (int i = 0; i < rows; i++) {
            String id  = String.valueOf(i);
            String carDealer = GenUtil.oneOf("Huat Huat Car", "Best Auto", "Fast Car Ptd Ltd", "Dealer X",
                    "Premium Auto", "Luxury Automobile", "Prestige Auto");
    ...
    public static CellStyle fontStyle(Workbook workbook, String fontname, int heightPoints, boolean isBold) {
