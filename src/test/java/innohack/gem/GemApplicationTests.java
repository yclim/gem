...
class GemApplicationTests {
  ...
  void testCSVParser() throws Exception {
    ...
    CsvFileGenerator.generateFixedCustomerCsvFiles(1, Paths.get("target/samples"), 100000);
    ...
    for (List<String> row : dataTable) {
      ...
      if (rowCount == 0) {
        for (int i = 0; i < row.size(); i++) {
          switch (i) {
            case 0:
              assert (row.get(i).equals("Customer ID"));
              break;
            case 1:
              assert (row.get(i).equals("Customer Name"));
              break;
            case 2:
              assert (row.get(i).equals("Gender"));
              break;
            case 3:
              assert (row.get(i).equals("Address"));
              break;
            case 4:
              assert (row.get(i).equals("Contact Number"));
              break;
            case 5:
              assert (row.get(i).equals("Email"));
              break;
            default:
              break;
          }
        ...
        for (int i = 0; i < row.size(); i++) {
          ...
          switch (i) {
            case 0:
              assert (row.get(i).equals(custIdInt + ""));
              break;
            case 1:
              assert (row.get(i).equals(name));
              break;
            case 2:
              assert (row.get(i).equals(gender));
              break;
            case 3:
              ...
              break;
            case 4:
              assert (row.get(i).equals(contactNumber));
              break;
            case 5:
              assert (row.get(i).equals(email));
              break;
